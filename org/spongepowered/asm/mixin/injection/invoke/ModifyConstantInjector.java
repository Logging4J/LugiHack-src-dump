...
public class ModifyConstantInjector extends RedirectInjector
{
    ...
    private void injectExpandedConstantModifier(final Target obj, final JumpInsnNode jumpInsnNode) {
        ...
        obj.replaceNode((AbstractInsnNode)jumpInsnNode, invokeConstantHandler, list);
        obj.addToStack(1);
    }
    ...
    private void checkNarrowing(final Target target, final AbstractInsnNode abstractInsnNode, final Type type, final Type type2, final int i, final String s) {
        final int sort = type.getSort();
        final int sort2 = type2.getSort();
        if (sort2 < sort) {
            Injector.logger.log((sort2 == 1) ? Level.ERROR : Level.WARN, "Narrowing conversion of <{}> to <{}> in {} target {} at opcode {} ({}){}", new Object[] { SignaturePrinter.getTypeName(type, false), SignaturePrinter.getTypeName(type2, false), this.info, target, i, s, (sort2 == 1) ? ". Implicit conversion to <boolean> can cause nondeterministic (JVM-specific) behaviour!" : "" });
        }
    }
}
