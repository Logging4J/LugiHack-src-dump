...
public class RedirectInjector extends InvokeInjector
{
    ...
    protected void validateParams(final RedirectedInvoke redirectedInvoke) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            final AnnotationNode invisibleParameter = Annotations.getInvisibleParameter(this.methodNode, (Class<? extends Annotation>)Coerce.class, i);
    ...
    protected boolean checkDescriptor(final String anObject, final Target target, final String s) {
        if (this.methodNode.desc.equals(anObject)) {
            return false;
        }
        final int index = anObject.indexOf(41);
        if (this.methodNode.desc.equals(String.format("%s%s%s", anObject.substring(0, index), Joiner.on("").join((Object[])target.arguments), anObject.substring(index)))) {
            return true;
        }
        throw new InvalidInjectionException(this.info, String.format("%s method %s %s has an invalid signature. Expected %s but found %s", this.annotationType, s, this, anObject, this.methodNode.desc));
    }
