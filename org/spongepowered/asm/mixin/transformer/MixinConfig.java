        ...
        for (final String s : this.injectorOptions.injectionPoints) {
            try {
                ...
                if (InjectionPoint.class.isAssignableFrom(class1)) {
                    InjectionPoint.register((Class)class1);
                }
                else {
        ...
        for (final String str : list) {
            ...
            if (str != null) {
                ...
                try {
                    ...
                    if (((MixinInfo)obj).getTargetClasses().size() <= 0) {
                        continue;
                    }
                    MixinConfig.globalMixinList.add(string);
                    final Iterator<String> iterator2 = ((MixinInfo)obj).getTargetClasses().iterator();
                    while (iterator2.hasNext()) {
                        final String replace = iterator2.next().replace('/', '.');
                        this.mixinsFor(replace).add((MixinInfo)obj);
                        this.unhandledTargets.add(replace);
                    }
                    final Iterator<IListener> iterator3 = this.listeners.iterator();
                    while (iterator3.hasNext()) {
                        iterator3.next().onPrepare((MixinInfo)obj);
                    }
                    this.mixins.add((MixinInfo)obj);
                }
                catch (InvalidMixinException ex) {
                    if (this.required) {
                        throw ex;
                    ...
                    if (this.required) {
                        throw new InvalidMixinException((IMixinInfo)obj, "Error initialising mixin " + obj + " - " + ex2.getClass() + ": " + ex2.getMessage(), ex2);
                    }
                    this.logger.error(ex2.getMessage(), (Throwable)ex2);
                }
            }
