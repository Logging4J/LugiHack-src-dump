...
class MixinPreProcessorStandard
{
    ...
    protected void prepareShadow(final MixinInfo.MixinMethodNode mixinMethodNode, final ClassInfo.Method method) {
        final AnnotationNode visible = Annotations.getVisible((MethodNode)mixinMethodNode, (Class<? extends Annotation>)Shadow.class);
        if (visible == null) {
            return;
        }
        final String prefix = Annotations.getValue(visible, "prefix", (Class<?>)Shadow.class);
        if (mixinMethodNode.name.startsWith(prefix)) {
            Annotations.setVisible((MethodNode)mixinMethodNode, (Class<? extends Annotation>)MixinRenamed.class, "originalName", mixinMethodNode.name);
            mixinMethodNode.name = method.renameTo(mixinMethodNode.name.substring(prefix.length()));
        }
    }
    ...
    protected void attachMethods(final MixinTargetContext mixinTargetContext) {
        final Iterator<MixinInfo.MixinMethodNode> iterator = (Iterator<MixinInfo.MixinMethodNode>)this.classNode.mixinMethods.iterator();
        while (iterator.hasNext()) {
            ...
            if (!this.validateMethod(mixinTargetContext, mixinMethodNode)) {
                ...
                mixinTargetContext.addMixinMethod((MethodNode)mixinMethodNode);
            }
                ...
                mixinTargetContext.addShadowMethod((MethodNode)mixinMethodNode);
                iterator.remove();
            }
    ...
    protected boolean attachAccessorMethod(final MixinTargetContext mixinTargetContext, final MixinInfo.MixinMethodNode mixinMethodNode, final SpecialMethod obj) {
        if (mixinMethodNode.getVisibleAnnotation((Class)obj.annotation) == null) {
            return false;
        }
        final String string = obj + " method " + mixinMethodNode.name;
        final ClassInfo.Method specialMethod = this.getSpecialMethod(mixinMethodNode, obj);
        if (MixinEnvironment.getCompatibilityLevel().isAtLeast(MixinEnvironment.CompatibilityLevel.JAVA_8) && specialMethod.isStatic()) {
            ...
            final String uniqueName = mixinTargetContext.getUniqueName((MethodNode)mixinMethodNode, true);
            ...
            mixinMethodNode.name = specialMethod.renameTo(uniqueName);
        }
        else {
            if (!specialMethod.isAbstract()) {
                throw new InvalidAccessorException((IMixinContext)mixinTargetContext, string + " is not abstract");
            }
            if (specialMethod.isStatic()) {
                throw new InvalidAccessorException((IMixinContext)mixinTargetContext, string + " cannot be static");
            }
        }
        mixinTargetContext.addAccessorMethod((MethodNode)mixinMethodNode, obj.annotation);
        return true;
    }
    ...
    protected boolean attachSpecialMethod(final MixinTargetContext mixinTargetContext, final MixinInfo.MixinMethodNode mixinMethodNode, final SpecialMethod specialMethod) {
        final AnnotationNode visibleAnnotation = mixinMethodNode.getVisibleAnnotation((Class)specialMethod.annotation);
    ...
    protected boolean attachUniqueMethod(final MixinTargetContext mixinTargetContext, final MixinInfo.MixinMethodNode mixinMethodNode) {
        ...
        if (method.isSynthetic()) {
            mixinTargetContext.transformDescriptor((MethodNode)mixinMethodNode);
        ...
        if (Bytecode.getVisibility((MethodNode)mixinMethodNode).ordinal() < Bytecode.Visibility.PUBLIC.ordinal()) {
            final String uniqueName = mixinTargetContext.getUniqueName((MethodNode)mixinMethodNode, false);
        ...
        if (this.strictUnique) {
            throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("Method conflict, %s method %s in %s cannot overwrite %s%s in %s", s, mixinMethodNode.name, this.mixin, method2.name, method2.desc, mixinTargetContext.getTarget()));
        }
        final AnnotationNode visible = Annotations.getVisible((MethodNode)mixinMethodNode, Unique.class);
        if (visible == null || !Annotations.getValue(visible, "silent", Boolean.FALSE)) {
            if (Bytecode.hasFlag((MethodNode)mixinMethodNode, 64)) {
                try {
                    Bytecode.compareBridgeMethods(method2, (MethodNode)mixinMethodNode);
                    ...
                    if (this.verboseLogging || this.env.getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
                        ex.printAnalysis((IMixinContext)mixinTargetContext, method2, (MethodNode)mixinMethodNode);
                    }
    ...
    protected void attachFields(final MixinTargetContext mixinTargetContext) {
        final Iterator<FieldNode> iterator = (Iterator<FieldNode>)this.classNode.fields.iterator();
        while (iterator.hasNext()) {
            final FieldNode fieldNode = iterator.next();
            final AnnotationNode visible = Annotations.getVisible(fieldNode, (Class<? extends Annotation>)Shadow.class);
            ...
            if (!this.validateField(mixinTargetContext, fieldNode, visible)) {
                ...
                if (fieldNode2 == null) {
                    ...
                    if (fieldNode2 == null) {
                        throw new InvalidMixinException((IMixinInfo)this.mixin, String.format("Shadow field %s was not located in the target class %s. %s%s", fieldNode.name, mixinTargetContext.getTarget(), mixinTargetContext.getReferenceMapper().getStatus(), getDynamicInfo(fieldNode)));
                    }
