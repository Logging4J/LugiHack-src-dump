...
public class MixinTargetContext extends ClassContext implements IMixinContext
{
    ...
    void addAccessorMethod(final MethodNode methodNode, final Class<? extends Annotation> clazz) {
        this.accessors.add(AccessorInfo.of(this, methodNode, (Class)clazz));
    }
    
    void addMixinMethod(final MethodNode methodNode) {
        Annotations.setVisible(methodNode, (Class<? extends Annotation>)MixinMerged.class, "mixin", this.getClassName());
    ...
    void methodMerged(final MethodNode methodNode) {
        ...
        Annotations.setVisible(methodNode, (Class<? extends Annotation>)MixinMerged.class, "mixin", this.getClassName(), "priority", this.getPriority(), "sessionId", this.sessionId);
    }
    ...
    public ClassInfo findRealType(final ClassInfo obj) {
        ...
        if (correspondingType == null) {
            throw new InvalidMixinException((IMixinContext)this, "Resolution error: unable to find corresponding type for " + obj + " in hierarchy of " + this.targetClassInfo);
        }
                ...
                if (obj.isDecoratedFinal()) {
                    if (obj.isDecoratedMutable()) {
                        ...
                        if (this.mixin.getParent().getEnvironment().getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
                            throw new InvalidMixinException((IMixinInfo)this.mixin, "Write access detected to @Final field " + obj + " in " + this.mixin + "::" + methodNode.name);
                        }
                    }
                }
            }
