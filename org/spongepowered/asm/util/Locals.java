...
public final class Locals
{
    ...
    public static LocalVariableNode[] getLocalsAt(final ClassNode classNode, final MethodNode methodNode, AbstractInsnNode nextNode) {
        ...
        for (final AbstractInsnNode abstractInsnNode : methodNode.instructions) {
            if (abstractInsnNode instanceof FrameNode) {
                ...
                a = ((frameData != null && frameData.type == 0) ? Math.min(a, frameData.locals) : frameNode.local.size());
                for (int n6 = 0, j = 0; j < array.length; ++j, ++n6) {
                    final Object o = (n6 < frameNode.local.size()) ? frameNode.local.get(n6) : null;
                    if (o instanceof String) {
                        ...
                        if (o != Opcodes.TOP) {
                            if (b) {
                                ...
                                if (!b2 && !b3) {
                                    throw new LVTGeneratorException("Unrecognised locals opcode " + o + " in locals array at position " + n6 + " in " + classNode.name + "." + methodNode.name + methodNode.desc);
                                }
                                array[j] = getLocalVariableAt(classNode, methodNode, nextNode, j);
                                if (b3) {
                                    ++j;
                        ...
                        if (o != null) {
                            throw new LVTGeneratorException("Invalid value " + o + " in locals array at position " + n6 + " in " + classNode.name + "." + methodNode.name + methodNode.desc);
                        }
                        if (j >= n4 && j >= a && a > 0) {
                            array[j] = null;
                        }
    ...
    private static boolean isOpcodeInRange(final InsnList list, final LocalVariableNode localVariableNode, final int n) {
        return list.indexOf((AbstractInsnNode)localVariableNode.start) < n && list.indexOf((AbstractInsnNode)localVariableNode.end) > n;
    }
    ...
    public static List<LocalVariableNode> getLocalVariableTable(final ClassNode classNode, final MethodNode methodNode) {
        ...
        return (List<LocalVariableNode>)methodNode.localVariables;
    }
