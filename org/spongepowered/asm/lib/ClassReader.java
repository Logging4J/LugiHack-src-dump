...
public class ClassReader
{
        ...
        while (true) {
            ...
            switch (desc.charAt(n++)) {
                ...
                case '[': {
                    ...
                    local[localCount++] = desc.substring(beginIndex, ++n);
                    continue;
                }
                case 'L': {
                    while (desc.charAt(n) != ';') {
                    ...
                    local[localCount++] = desc.substring(beginIndex + 1, n++);
                    continue;
                }
                default: {
    ...
    private Attribute readAttribute(final Attribute[] array, final String anObject, final int n, final int n2, final char[] array2, final int n3, final Label[] array3) {
        for (int i = 0; i < array.length; ++i) {
            if (array[i].type.equals(anObject)) {
                return array[i].read(this, n, n2, array2, n3, array3);
            }
        }
        return new Attribute(anObject).read(this, n, n2, (char[])null, -1, (Label[])null);
    }
    ...
    private String readUTF(int i, final int n, final char[] value) {
        final int n2 = i + n;
        final byte[] b = this.b;
        int count = 0;
        int n3 = 0;
